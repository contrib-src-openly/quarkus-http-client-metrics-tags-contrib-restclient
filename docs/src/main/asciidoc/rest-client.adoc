...
The REST Client can connect to HTTP/3 servers when the `quarkus-http3` extension is on the classpath.
...
See the xref:http3-reference.adoc[HTTP/3 reference guide] for server-side setup and additional details.
...
All declarative REST Client instances produce request metrics using the `http.client.requests` timer.
The metrics contain a tag named `clientName` which corresponds to the config key of the client, as specified by the `configKey` property of the `@RegisterRestClient` annotation.
When the xref:telemetry-micrometer.adoc[`quarkus-micrometer`] extension is used, CDI beans that implement xref:telemetry-micrometer.adoc#http-client-metrics-tags-contributor[`HttpClientMetricsTagsContributor`] can add custom tags to these request metrics.
